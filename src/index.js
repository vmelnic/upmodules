...
exports.prepare = function (obj, globalOpts) {
    return Object.keys(obj).map(function (key) {
        ...
        if (!command || !output) {
            console.error('Cannot parse command', line);
            return;
        }

        if (globalOpts.allow) {
            if (!globalOpts.allow.includes(command)) {
                console.error('Unauthorized command', command);
                return;
            }
        }
...
function processCommand(obj, opts) {
    ...
    if (!srcPath) {
        srcPath = Path.resolve(opts.cwd, obj.input);
    }

    var srcFile = Path.basename(srcPath);
    var commandFn;

    if (commands.includes(obj.command)) {
        commandFn = require(`../commands/${obj.command}`);
    } else {
        commandFn = require(resolveFrom(opts.cwd, `upmodules-${obj.command}`));
    }
    ...
    return fs.ensureDir(destDir).then(function () {
        return glob(srcPath, {
            ...
            if (paths.length === 0 && obj.options.length !== 0) {
                paths.push(obj.input);
            }

            if (paths.length === 0) {
                throw new Error('No files found at ' + srcPath);
            }
            ...
            if (bundle || obj.options.list) {
                return commandFn(list, obj.output, obj.options);
            }
